...
def hr(colour=NAVY, thickness=1.5):
    return HRFlowable(width="100%", thickness=thickness, color=colour, spaceAfter=5)


def section_rule(colour=NAVY):
    return HRFlowable(width="100%", thickness=0.5, color=colour, spaceBefore=2, spaceAfter=8)
...
def generate_pdf(
    ...
    doc = BaseDocTemplate(
        ...
        title=title, author="ClinicalDocsPro"
    )
